...
#drn_d_105 = DRN(Bottleneck, [1, 1, 3, 4, 23, 3, 1, 1], arch='D', out_middle=out_middle, **kwargs)
class DRN(nn.Module):
    def __init__(self, block, layers, num_classes=1000,
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):  # 对卷积层权重初始化
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
            elif isinstance(m, BatchNorm):  # 对bn层权重和偏置初始化
...
def drn_d_105(pretrained=False, out_middle=False, **kwargs):
    model = DRN(Bottleneck, [1, 1, 3, 4, 23, 3, 1, 1], arch='D', out_middle=out_middle, **kwargs)
    if pretrained:
